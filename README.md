...
- 新建轨迹：右侧列表可管理多条轨迹
- 点击画面不会加点。Control（或 ⌘）拖动为框选，Control+点击为正点，Shift+Control+点击为负点
- 跟踪模式下按住左键拖动画面可以平移；触控板双指拖动同样平移，捏合或鼠标滚轮缩放，工具栏缩放按钮还原
- 函数图：单击跳到对应帧；双指、右键或按住 Alt 拖动平移；滚轮或捏合缩放，双击复位
...
每一项小改动单独开一个分支，只提交这一项，推到 GitHub 并开 Pull Request。对方拉取该分支后按上面的「运行」装上即可。不要把无关的未提交改动打进同一次提交。
